export function SplitMark({ className }: { className?: string }) {
  return (
    <svg
      viewBox="0 0 32 32"
      className={className}
      aria-hidden="true"
      shapeRendering="crispEdges"
    >
      <rect width="32" height="32" fill="#4f6ef7" />
      <rect x="6" y="6" width="4" height="4" fill="#ffffff" />
      <rect x="10" y="6" width="4" height="4" fill="#ffffff" />
      <rect x="6" y="10" width="4" height="4" fill="#ffffff" />
      <rect x="6" y="14" width="4" height="4" fill="#c5cffc" />
      <rect x="6" y="18" width="4" height="4" fill="#c5cffc" />
      <rect x="6" y="22" width="4" height="4" fill="#ffffff" />
      <rect x="10" y="22" width="4" height="4" fill="#ffffff" />
      <rect x="18" y="6" width="4" height="4" fill="#ffffff" />
      <rect x="22" y="6" width="4" height="4" fill="#ffffff" />
      <rect x="18" y="10" width="4" height="4" fill="#c5cffc" />
      <rect x="22" y="14" width="4" height="4" fill="#c5cffc" />
      <rect x="18" y="18" width="4" height="4" fill="#c5cffc" />
      <rect x="18" y="22" width="4" height="4" fill="#ffffff" />
      <rect x="22" y="22" width="4" height="4" fill="#ffffff" />
    </svg>
  );
}
